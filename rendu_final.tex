...
\section{Système directionel initial}
...
\begin{center}
  ...
  \end{tabular}
\end{center}
\caption{\label{l293dMax}Spécifications max du l293d:}
Informations tirées des fiches téchniques publiées
par Texas Instruments \cite{l293dDataSheet}.
\end{table}


\subsubsection{Fonctionnement}

Le chip fonctionne sur les mêmes principes que le circuit illustré
précédemment(\ref{H-bridge}). Il a en plus l'avantage d'être un double
H-bridge ce qui sera utile plus tard. Son comportement en fonction du signal
reçu peut également être illustré grâce au tableau de vérité
(\ref{tableDeVerite}). Le premier circuit dépend de INPUT1 (In1), INPUT2
(In2), OUTPUT1 (Out1) et OUTPUT2 (Out2), le deuxième se trouve du côté opposé
et a un comportement symétrique.
