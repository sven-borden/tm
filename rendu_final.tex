...
Guvcview est un software permettant d'enregistrer des séquences vidéos ou des images, il fourni aussi une image de contrôle, que nous utilisions durant une bonne partie du projet afin d'avoir l'image. Le point faible de ce logiciel et qu'avec celui-ci, le nombre d'images par seconde est faible, trois à quatre images. Avec d'autres logiciels, tel que OpenCV permet d'obtenir plus de 25 images par seconde. 
\clearpage

\chapter{Java \& Python}
\lettrine{D}{ans} notre projet, nous avons commencé par faire nous programmes en python avant de recommencer en Java, bien que le code en python fonctionne. 

\section{Python}
Notre code python (annexe: \ref{code python}) est dans la mémoire du Raspberry Pi et lorsqu'on éxécute ce code, il ouvre une fenêtre (fig:\ref{python environnement}) où nous pouvons interéagir avec l'arduino.
